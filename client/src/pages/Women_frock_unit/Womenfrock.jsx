...
import "../Men_shirt_unit/menshirts.css"
export const Womenfrock=()=>{
    const {WomenFrock,rating_mob,sizes_shirt,color_men_shirt,fabric_men_shirt}=useContext(AuthContext)

    const handleSingleShirt=(item)=>{
        window.open(`/detailswomenfrock/${item._id}`)
    }

    const filterWomenFrock=WomenFrock.filter((curElem)=>{
    ...
    const displayFilter=filterWomenFrock.length>0?filterWomenFrock:WomenFrock
                    ...
                    {displayFilter.map((curElem,index)=>(
                        <div onClick={()=>handleSingleShirt(curElem)} style={{cursor:'pointer'}} >
                            <li key={index} id="men_shirt_container">
                                <div>
                                    <img src={curElem.img_women} id="main_img_men"/>
                                </div>
